...
namespace ImplicitNullability.Samples.CodeWithIN.Highlighting.ImplicitNotNullConflictInHierarchy
{
    public class NullabilityInheritanceBehavior
    {
        ...
        public class AsyncImplementation : ITaskInterface
        {
            ...
            {
                await Task.Delay(0);
                return null /*Expect:AssignNullToNotNullAttribute[RS >= 92]*/;
            }

            public async Task<string> ExplicitCanBeNullInBase /*Expect:ImplicitNotNullElementCannotOverrideCanBeNull[RS >= 92 && Implicit]*/()
            {
                await Task.Delay(0);
                return null;
            }
